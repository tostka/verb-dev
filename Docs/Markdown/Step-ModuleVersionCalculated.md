...
```
Step-ModuleVersionCalculated [-Path] <String> [[-Method] <String>] [-MinVersionIncrement]
 [[-MinVersionIncrementBump] <Object>] [-applyChange] [-Silent] [-whatIf] [<CommonParameters>]
```
...
### EXAMPLE 1
```
Step-ModuleVersionCalculated -path 'C:\sc\Get-MediaInfo' -whatif -verbose ;
```
...
### EXAMPLE 2
```
Step-ModuleVersionCalculated -path 'C:\sc\Get-MediaInfo'  -verbose -NoBuildInfo ;
```
...
### EXAMPLE 3
```
Step-ModuleVersionCalculated -path 'C:\sc\Get-MediaInfo'  -verbose -NoBuildInfo -applyChange ;
```
...
### EXAMPLE 4
```
Step-ModuleVersionCalculated -path 'C:\sc\Get-MediaInfo'  -verbose -Method Percentage -applyChange ;
```
...
### EXAMPLE 5
```
$newRevBump = Step-ModuleVersionCalculated -path 'C:\sc\Get-MediaInfo' ;
```
...
* 10:56 AM 5/20/2022 WIP: add: validator for ...
; -MinVersionIncrementBump (coerce Min fail through rev to Build; or use as explicit step driver, constant, rather than hard-coded in code) ; 
    ...
    tried running AST profiling to pull functions & aliases, but takes _3Mins_ to run.
Simpler, and 90% effective to do an sls parse.
...
Requires -RunasAdministrator](https://powershellexplained.com/2017-10-14-Powershell-module-semantic-version/
